...
import { updateCharacterVisual } from '@/actions/updateProjectVisuals';
...
export default function CharacterEditor({ projectId, initialValue, initialValueVi, language = 'Vietnamese' }: CharacterEditorProps) {
    ...
    const handleSave = async () => {
        setSaving(true);
        const result = await updateCharacterVisual(projectId, value, valueVi);
        setSaving(false);
        ...
        <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                    {isBilingual ? '🌐 Character Visual (Bilingual)' : '👤 Character Visual Base'}
                </h3>
                {!isEditing ? (
                    <button
            ...
            {isEditing ? (
                    ...
                    {isBilingual && (
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder={`Character description in ${language}...`}
                            />
                        </div>
                    )}
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                            🇻🇳 {isBilingual ? 'Vietnamese Translation' : 'Mô tả nhân vật'}
                        </label>
                        <textarea
                            value={isBilingual ? valueVi : value}
                            onChange={(e) => isBilingual ? setValueVi(e.target.value) : setValue(e.target.value)}
                            className="w-full h-32 px-3 py-2 border border-gray-300 rounded-md text-gray-900 bg-gray-50 focus:ring-2 focus:ring-blue-500"
                            placeholder="Mô tả chi tiết nhân vật chính..."
                        />
                    </div>
                </div>
            ) : (
                <div className="space-y-4">
                    ...
                    <div className={isBilingual ? "bg-green-50 p-4 rounded-lg border border-green-200" : ""}>
                        ...
                        <p className="text-gray-800 whitespace-pre-wrap text-sm">
                            {(isBilingual ? valueVi : value) || 'Chưa có mô tả nhân vật'}
                        </p>
                    </div>
                </div>
            )}
